...
def updateBatch(solution, batchData):
	try:
		currentBatchData = Tool.readJsonMultilingual(os.path.join(solution, batchData["id"] + ".json"))
...
def getSource(corePath, batchName):
	checkSourcePath = Tool.readJsonZip(corePath, "product-system-biz-unit.zip", batchName)
	return checkSourcePath


def updateProperties(solution, autoRegisterApi):
	propertiesPath = Tool.createPath(solution, 'srv-engage\\src\\main\\resources\\config\\pserver.service.properties')
	enabled = True
	method = True
...
def updateContexts(solution, context, groupName):
	...
	contextJson = Tool.readJson(insightsContextsPath)
	...
	Tool.updateJson(insightsContextsPath, contextJson)
...
def overrideBatch(solution, overrideFile, qaCopy, method):
	...
	if method == "Modify":
		Tool.updateJson(filePath, tmpOverrideFile)
	else:
		Tool.writeJsonMultiLang(filePath, tmpOverrideFile)

	if qaCopy:
		qaPath = os.path.join(solution, tmpOverrideFile['id'] + '-qa.json')
		if method == "Modify":
			sourceFile = Tool.readJson(qaPath)
		...
		if method == "Modify":
			Tool.updateJson(qaPath, tmpOverrideFile)
		else:
			Tool.writeJsonMultiLang(qaPath, tmpOverrideFile)
...
def main(argv):
	Tool.startLog()
	try:
		solution = Tool.createPath(Tool.getSolution(Tool.getPath('solution')), 'Batch\\SBatch')
	except Exception as e:
		error('Path Error:' + e.__str__()[e.__str__().index(']') + 1:])
		return
	try:
	...
	if method == "New Batch":
		...
		if os.path.exists(os.path.join(solution, argv[0]["id"] + ".json")):
			if adhoc:
				sourceFile = Tool.readJson(os.path.join(solution, argv[0]["id"] + "-adhoc.json"))
			else:
				sourceFile = Tool.readJson(os.path.join(solution, argv[0]["id"] + ".json"))
		else:
			showwarning("No batch", "You choosed Modify for method but there's nothing to modify.\nPlease change method and press the RUN button.")
			return

	Tool.prettyPrintJson(argv[0])
	batch = tmpBatch(argv[0], sourceFile)
	Tool.prettyPrintJson(batch)
	overrideBatch(solution, batch, qa, method)

	if "autoRegister" in argv[0]['importMethods']:
		updateProperties(Tool.getPath('solution'), argv[0]['API'])
		updateContexts(Tool.getSolution(Tool.getPath('solution')), argv[0]['context'], argv[0]['groupName'])
